...
export const registerTelegramHandlers = ({
  ...
  const inboundDebouncer = createInboundDebouncer<TelegramDebounceEntry>({
    ...
    onFlush: async (entries) => {
      ...
      const messageIdOverride =
        last.msg.message_id != null ? String(last.msg.message_id) : undefined;
  ...
  const processInboundMessage = async (params: {
    ...
    if (text && !isCommandLike) {
      ...
      if (existing) {
        ...
        if (canAppend) {
          const currentChars = existing.messages.reduce(
        ...
        textFragmentProcessing = textFragmentProcessing
          .then(() => flushTextFragments(existing))
          .catch(() => {});
      }
      ...
      if (existing) {
        ...
        existing.timer = setTimeout(() => {
          ...
          mediaGroupProcessing = mediaGroupProcessing
            .then(() => processMediaGroup(existing))
            .catch(() => {});
        }, mediaGroupTimeoutMs);
      } else {
    ...
    try {
      ...
      if (String(mediaErr).includes("MB limit") && sendOversizeWarning) {
        await bot.api
          .sendMessage(
            chatId,
            `⚠️ File too large. Maximum is ${Math.round(mediaMaxBytes / 1048576)}MB.`,
    ...
    const debounceKey = msg.from?.id
      ? `telegram:${accountId ?? "default"}:${conversationKey}:${msg.from.id}`
      : null;
  ...
  bot.on("message_reaction", async (ctx) => {
    try {
      ...
      const reactionMode = telegramCfg.reactionNotifications ?? "own";
      if (reactionMode === "off" || reaction.user?.is_bot) {
        return;
      }
      if (reactionMode === "own" && !wasSentByBot(reaction.chat.id, reaction.message_id)) {
        return;
      }
      ...
      const added = reaction.new_reaction.filter(
        (r): r is ReactionTypeEmoji => r.type === "emoji" && !oldEmojis.has(r.emoji),
      );
      if (added.length === 0) {
        return;
      }
      ...
      const resThreadId = reaction.chat.is_forum
        ? resolveTelegramForumThreadId({ isForum: true, messageThreadId: undefined })
        : undefined;
      const route = resolveAgentRoute({
        cfg: loadConfig(),
        channel: "telegram",
        ...
        peer: {
          ...
          id: isGroup
            ? buildTelegramGroupPeerId(reaction.chat.id, resThreadId)
            : String(reaction.chat.id),
        },
      });

      for (const r of added) {
        enqueueSystemEvent(`Telegram reaction: ${r.emoji} by ${label}`, {
          sessionKey: route.sessionKey,
          contextKey: `react:${reaction.chat.id}:${reaction.message_id}:${user?.id}:${r.emoji}`,
        });
      }
    } catch (err) {
      runtime.error?.(danger(`Reaction handler failed: ${String(err)}`));
    }
  });

  bot.on("callback_query", async (ctx) => {
    const callback = ctx.callbackQuery;
    if (!callback || !callback.message || !("chat" in callback.message)) {
      return;
    }
    if (shouldSkipUpdate(ctx)) {
      return;
    ...
    try {
      ...
      if (inlineButtonsScope === "allowlist") {
        const allow = isGroup
          ? effectiveGroupAllow
          : normalizeAllowFromWithStore({ allowFrom: telegramCfg.allowFrom, storeAllowFrom });
        if (!isAllowlistAuthorized(allow, senderId, senderUsername)) {
          return;
        }
      }
      ...
      if (paginationMatch) {
        ...
        const result = buildCommandsMessagePaginated(
          cfg,
          listSkillCommandsForAgents({ cfg, agentIds: agentId ? [agentId] : undefined }),
          { page, surface: "telegram" },
        );
        const keyboard =
          result.totalPages > 1
            ? buildInlineKeyboard(
      ...
      if (modelCallback) {
        ...
        if (modelCallback.type === "providers" || modelCallback.type === "back") {
          await editOrReply(
            "Select a provider:",
            buildProviderKeyboard(
              providers.map((p) => ({ id: p, count: byProvider.get(p)?.size ?? 0 })),
            ),
          );
        } else if (modelCallback.type === "list") {
          const models = [...(byProvider.get(modelCallback.provider) ?? [])].toSorted();
          const current = resolveTelegramSessionModel({
            chatId,
            isGroup,
            isForum: msg.chat.is_forum === true,
            messageThreadId: msg.message_thread_id,
          ...
          await editOrReply(
            `Models (${modelCallback.provider})`,
            buildModelsKeyboard({
              provider: modelCallback.provider,
              models,
              currentModel: current,
              currentPage: modelCallback.page,
              totalPages: calculateTotalPages(models.length, getModelsPageSize()),
          ...
          await processMessage(
            buildSyntheticContext(
              ...
              buildSyntheticTextMessage({
                base: msg,
                from: callback.from,
                text: `/model ${modelCallback.provider}/${modelCallback.model}`,
              }),
            ),
            [],
            storeAllowFrom,
      ...
      if (modelPickMatch) {
        ...
        const route = resolveAgentRoute({
          ...
          peer: {
            kind: isGroup ? "group" : "direct",
            id: isGroup ? buildTelegramGroupPeerId(chatId, msg.message_thread_id) : String(chatId),
          },
        });
        const storePath = resolveStorePath(cfg.session?.store, { agentId: route.agentId });
        const store = loadSessionStore(storePath);
        ...
        await ctx
          .editMessageText(`Use <b>${modelKey}</b>.`, { parse_mode: "HTML" })
          .catch(() => {});
        return;
      }
      ...
      if (modelPageMatch) {
        ...
        const currentModel = store[route.sessionKey]?.providerOverride
          ? `${store[route.sessionKey].providerOverride}/${store[route.sessionKey].modelOverride}`
          : undefined;
        const picker = await buildModelPickerMessage({
          cfg,
          page: parseInt(modelPageMatch[2], 10),
          provider: modelPageMatch[1],
          currentModel,
          agentId: route.agentId,
        });
        await ctx
          .editMessageText(picker.text, { parse_mode: "HTML", reply_markup: picker.reply_markup })
          .catch(() => {});
        return;
      }
      ...
      await processMessage(
        buildSyntheticContext(ctx, { ...msg, from: callback.from, text: data } as Message),
        [],
        storeAllowFrom,
        { forceWasMentioned: true, messageIdOverride: callback.id },
      );
    } catch (err) {
      runtime.error?.(danger(`Callback query handler failed: ${String(err)}`));
    }
  });
  ...
  bot.on("message:migrate_to_chat_id", async (ctx) => {
    try {
      ...
      runtime.error?.(danger(`Group migration failed: ${String(err)}`));
    }
  });
    ...
    try {
      ...
      const route = resolveAgentRoute({
        ...
        peer: {
          kind: isGroup ? "group" : "direct",
          id: isGroup
            ? buildTelegramGroupPeerId(ctx.chat.id, ctx.message?.message_thread_id)
            : String(ctx.chat.id),
        },
      });
      const store = loadSessionStore(
      ...
      const message = await buildProviderPickerMessage({
        cfg,
        currentProvider: store[route.sessionKey]?.providerOverride,
      });
      ...
      runtime.error?.(danger(`Models command failed: ${String(err)}`));
    }
  });
  ...
  bot.on("message", async (ctx) => {
    try {
      ...
      await processInboundMessage({
        ...
        msg,
        chatId: msg.chat.id,
        resolvedThreadId: groupContext.resolvedThreadId,
        storeAllowFrom: groupContext.storeAllowFrom,
        sendOversizeWarning: true,
        oversizeLogMessage: "media exceeds limit",
      });
    } catch (err) {
      runtime.error?.(danger(`Message handler failed: ${String(err)}`));
    }
  });
  ...
  bot.on("channel_post", async (ctx) => {
    try {
      ...
      if (
        (resolveGroupPolicy(post.chat.id).allowlistEnabled &&
          !resolveGroupPolicy(post.chat.id).allowed) ||
        groupContext.groupConfig?.enabled === false
      ) {
        return;
      }

      const syntheticFrom = post.sender_chat
        ? { id: post.sender_chat.id, is_bot: true, first_name: post.sender_chat.title || "Channel" }
        : { id: post.chat.id, is_bot: true, first_name: "Channel" };
      const syntheticMsg = {
        ...post,
        from: post.from ?? syntheticFrom,
        chat: { ...post.chat, type: "supergroup" },
      } as Message;
      ...
      await processInboundMessage({
        ...
        oversizeLogMessage: "channel post media",
      });
    } catch (err) {
      runtime.error?.(danger(`Channel handler failed: ${String(err)}`));
    }
  });
};
